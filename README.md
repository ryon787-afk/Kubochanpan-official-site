...
- 管理画面ホームの一番上に「注文通知」を表示
- 管理画面ホームにサイト写真を表示
- サイト編集・画像管理・バナー管理を強化
- 商品ごとの複数写真を見やすいギャラリー表示に変更
- 注文画面から配送時間を削除
- カート追加時に毎回「カートに追加しました」を表示
- Supabase注文保存設定を追加
- 水色・白・ナチュラル木目調のデザインを追加

## Supabase注文保存

管理画面の「設定」で以下を入力してください。
...
`supabase-setup.sql` をSupabase SQL Editorで実行すると、注文保存用テーブルを作成できます。

個人情報保護のため、公開読み取りポリシーは作っていません。
